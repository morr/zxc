    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.debug_struct("PlantedState")
            .field(
                ...
                &format_args!(
                    "{:.2}s",
                    TimerWrapper(&planted_state.growth_timer).remaining_secs()
                ),
            )
            .field(
                "tending_rest_timer",
                &format_args!(
                    "{:.2}s",
                    TimerWrapper(&planted_state.tending_rest_timer).remaining_secs()
                ),
            )
            .field(
                "tending_rest_started_day",
                &planted_state.tending_rest_started_day,
            )
            .field(
                "is_tending_pending_for_next_day",
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.debug_struct("HarvestedState")
            .field(
                ...
                &format_args!(
                    "{:.2}s",
                    TimerWrapper(&harvested_state.rest_timer).remaining_secs()
                ),
            )
            .finish()
    }
