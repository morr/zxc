...
pub fn spawn_map(
    ...
    for x in -get_config().grid.half_size..get_config().grid.half_size {
        for y in -get_config().grid.half_size..get_config().grid.half_size {
            ...
            let id = commands
                .spawn(SpriteBundle {
                    ...
                    sprite: Sprite {
                        custom_size: Some(Vec2::new(get_config().tile.size, get_config().tile.size)),
                        ..default()
                    },
                    transform: Transform::from_xyz(
                        grid_tile_edge_to_world(x) + get_config().tile.size / 2.,
                        grid_tile_edge_to_world(y) + get_config().tile.size / 2.,
                        TILE_Z_INDEX,
                    ),
                    ..default()
                })
